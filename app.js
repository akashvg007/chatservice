...
io.on("connection", (socket) => {
  ...
  socket.on("statusUpdate", ({ receiver, time, sender }) => {
    console.log("statusUpdate", receiver, time, sender);
    socket.broadcast
      .to(receiver)
      .emit("status-update-last", { receiver, time, sender });
  });
  ...
  socket.on("send-message", ({ recipient, text }) => {
    ...
    socket.broadcast
      .to(recipient)
      .emit("receive", { recipient, sender: id, text });
